...
const BOT_INVITE_URL =
    "https://discord.com/oauth2/authorize?client_id=1484089682655580231&integration_type=0&scope=applications.commands";

export function renderUninstall(root: HTMLElement): void {
    root.innerHTML = `
    ...
    bindStepIndicator(root, (step) => {
        ...
        else if (step === 2) showAccounts(root);
    });

    root.querySelectorAll<HTMLButtonElement>(".qa-question").forEach((btn) => {
    ...
    root.querySelectorAll<HTMLAnchorElement>(".uninstall-link").forEach((link) => {
    ...
    root.querySelector<HTMLButtonElement>("#btn-uninstall")!.addEventListener("click", async () => {
        try {
            await uninstall();
        } catch (err) {
